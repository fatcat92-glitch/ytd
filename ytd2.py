# YouTube Downloader GUI - FULL VERSION (No more Shorts tab error + Shorts in separate folder + 1080p Balanced default)
# Requirements: pip install yt-dlp pillow
# ffmpeg required (brew install ffmpeg on Mac)
...
class YouTubeDownloader:
    def __init__(self, root):
        ...
        # Default to folder where the executable lives
        if getattr(sys, 'frozen', False):
            self.download_dir = os.path.dirname(sys.executable)
        else:
    ...
    def setup_single_tab(self):
        ...
        self.quality_var = tk.StringVar(value="1080p")  # Balanced default
    ...
    def download_single(self):
        # unchanged (your working single video code)
        url = self.single_url_var.get().strip()
        if not url:
    ...
    # ===================== CHANNEL DOWNLOAD - FIXED (no Shorts tab error) =====================
        ...
        def dl_bulk():
            try:
                opts_extract = {
                    ...
                    'extract_flat': True,
                    'ignoreerrors': True,
                    'extractor_args': {'youtube': {'skip': ['dash', 'hls']}}  # suppresses JS runtime warning
                }
                video_entries = []
                shorts_entries = []
                ...
                # Shorts - now safely ignored if no tab exists
                if mode in ('shorts', 'both'):
                    ...
                    try:
                        ...
                        pass  # channel has no Shorts tab → just skip silently
                ...
                for i, entry in enumerate(video_entries, 1):
                    ...
                    self.root.after(0, lambda t=f"Downloading video {i} of {total}: {title}": self.channel_prog_label.config(text=t))

                    opts = {
                        'format': fmt,
                        'outtmpl': os.path.join(base_out, '%(title)s.%(ext)s'),
                        'merge_output_format': 'mp4',
                        'continuedl': True,
                        'ignoreerrors': True,
                    }
                ...
                for i, entry in enumerate(shorts_entries, 1):
                    ...
                    self.root.after(0, lambda t=f"Downloading short {i} of {total}: {title}": self.channel_prog_label.config(text=t))

                    opts = {
                        'format': fmt,
                        'outtmpl': os.path.join(shorts_out, '%(title)s.%(ext)s'),
                        'merge_output_format': 'mp4',
                        'continuedl': True,
                        'ignoreerrors': True,
                    }
                    ...
                    percent = ((offset + i) / total) * 100 if total else 0
                    self.root.after(0, lambda p=percent: self.channel_progressbar.config(value=p))
